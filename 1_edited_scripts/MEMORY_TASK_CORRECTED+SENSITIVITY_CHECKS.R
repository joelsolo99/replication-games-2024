...
t.test(x = agg_response_df$accuracy[agg_response_df$orientation == "facing"], 
       y = agg_response_df$accuracy[agg_response_df$orientation == "away"], 
       paired = TRUE, 
       var.equal = TRUE)
lsr::cohensD(
  x = agg_response_df$accuracy[agg_response_df$orientation == "facing"],
  y = agg_response_df$accuracy[agg_response_df$orientation == "away"],
  method = "paired"
)
...
response_df_RT%>%group_by(ID,orientation)%>%summarise(RT=mean(Time))->response_df_RT
(response_df_RT%>%group_by(orientation)%>%
    summarise(N=n(), M=mean(RT), sd=sd(RT),
              sem=goeveg::sem(RT))%>%rename(RT=M) -> desc_stats)
t.test(x = response_df_RT$RT[response_df_RT$orientation == "facing"], 
       y = response_df_RT$RT[response_df_RT$orientation == "away"], 
       paired = TRUE, 
       var.equal = TRUE)
lsr::cohensD(
  x = response_df_RT$RT[response_df_RT$orientation == "facing"],
  y = response_df_RT$RT[response_df_RT$orientation == "away"],
  method = "paired"
)
ggplot(response_df_RT, aes(x=orientation, y=RT)) +
  geom_histogram(position='dodge', stat='summary', aes(fill=orientation, alpha=ID)) +
  geom_point(data=desc_stats, aes(stat='summary')) +
...
#
##SENSITIVITY CHECKS##
#Check for normal distribution of accuracy by performing Shapiro-Wilk test split by group
shapiro_by_group <- function(data, group_vars = c("orientation"), value_var = "accuracy") {
  data %>%
...
shapiro_results <- shapiro_by_group(
  data = agg_response_df,
  group_vars = c("orientation"),
  value_var = "accuracy"
)
print(shapiro_results)

#Run non-parametric tests: Wilcoxon signed-rank test on accuracy data
wide_agg_response_df <- agg_response_df %>%
  pivot_wider(names_from = "orientation", values_from = "accuracy")
wilcox.test(wide_agg_response_df$facing, wide_agg_response_df$away, paired = TRUE)

#Check for normal distribution of reaction times by performing Shapiro-Wilk test split by group
shapiro_by_group <- function(data, group_vars = c("orientation"), value_var = "RT") {
  data %>%
    group_by(across(all_of(group_vars))) %>%
    summarise(
      p_value = shapiro.test(!!sym(value_var))$p.value,     
...
#Run non-parametric tests: Wilcoxon signed-rank test on reaction time data
wide_response_df_RT <- response_df_RT %>%
  pivot_wider(names_from = "orientation", values_from = "RT")
wilcox.test(wide_response_df_RT$facing, wide_response_df_RT$away, paired = TRUE)
