...
Wide_DF <- diff_df #saved that version of diff_df
...
#sSTART OF OUR ANALYSIS
#sensitivity checks

#paired t.test 
unique(diff_df$Subject)

A<-diff_df %>% group_by(Group, Subject) %>% summarise(mean_facing = mean(facing))
B<-diff_df %>% group_by(Group, Subject) %>% summarise(mean_facing_away = mean(facing_away))
library(data.table)
diff_group <- merge(A,B, by='Subject')
mean(diff_group$mean_facing)
mean(diff_group$mean_facing_away)
t.test(diff_group$mean_facing,diff_group$mean_facing_away, alternative="two.sided", paired=TRUE)
cohensD(diff_group$mean_facing,diff_group$mean_facing_away)


install.packages('lsr')
library(lsr)

cohensD(diff_df$Difference,
        mu = 0) #the cohen's d replicates
...
library(lme4)
library(lmerTest)
pivoted_df <- pivot_longer(Wide_DF, cols=c("facing","facing_away"), values_to = "Duration",names_to="Direction")

model1 <- lmer(Duration ~ Direction + (1|Trial_Subject) + (1 | Subject), data=pivoted_df)
summary(model1)
