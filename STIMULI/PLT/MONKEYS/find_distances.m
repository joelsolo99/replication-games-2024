%% LP: overlay an image on a background
stimuliPath='C:\Users\ngoupil\ownCloud2\Liuba_Team\Projects\PLTs\monkeys\monkey_stims\natural\pairs\facing';
outputPath='C:\Users\ngoupil\ownCloud2\Liuba_Team\Projects\PLTs\monkeys\monkey_stims\natural\pairs\facing';
listOfJpegs = dir('C:\Users\ngoupil\ownCloud2\Liuba_Team\Projects\PLTs\monkeys\monkey_stims\natural\pairs\facing\*png');

% %% LP: overlay an image on a background
% stimuliPath='C:\Users\eabassi\Documents\Scripts\combine_flip\Facing_away\';
% outputPath='C:\Users\eabassi\Documents\Scripts\combine_flip\Facing_away\';
% listOfJpegs = dir('C:\Users\eabassi\Documents\Scripts\combine_flip\Facing_away\*.png');
...
% end
    ...
    [r c z] = size(a);
    for q=1:c
%         if any(find((a(:,q,1)<255) | a(:,q,2)<255 | a(:,q,3)<255)); %3d color images
        if any(find(a(:,q,1)<255))
            q_start1=q;
            break
        end
    end
    for q=q_start1+1:c
%         if isempty(find((a(:,q,1)<255) | a(:,q,2)<255 | a(:,q,3)<255));
        if isempty(find(a(:,q,1)<255))% | a(:,q,2)<255 | a(:,q,3)<255));
            q_end1=q;
            break
        end   
...
%     xtrans=round(((c/2)-q_center));
    ...
    for q=q_end1+1:c
%         if any(find((a(:,q,1)<255) | a(:,q,2)<255 | a(:,q,3)<255));
        if any(find(a(:,q,1)<255))% | a(:,q,2)<255 | a(:,q,3)<255));
            q_start2=q;
            break
        end
...
%     [r c z] = size(C);
%     for q=1:c
%         if any(find((C(:,q,1)<255) | C(:,q,2)<255 | C(:,q,3)<255));
%             q_start=q;
%             break
%         end
%     end
%     for q=q_start+1:c
%         if isempty(find((C(:,q,1)<255) | C(:,q,2)<255 | C(:,q,3)<255));
%             q_end=q;
%             break
%         end   
%     end
% 
%     for p=1:r
%         if any(find((C(p,:,1)<255) | C(p,:,2)<255 | C(p,:,3)<255));
%             p_start=p;
%             break
%         end
%     end
%     for p=p_start+1:r
%         if isempty(find((C(p,:,1)<255) | C(p,:,2)<255 | C(p,:,3)<255));
%             p_end=p;
%             break
%         end   
%     end
